...
async fn test_action_claim_new_provider_success() {
    ...
    let res = service
        .action(Request::new(ActionRequest {
            message: Some(action),
        }))
        .await;
    assert!(res.is_ok());
...
async fn test_action_key_theft_denied() {
    ...
    assert!(
        res.unwrap_err()
            .message()
            .contains("already associated with another provider")
    );
}
...
async fn test_get_ledger_mapping() {
    ...
    let ledger = service
        .get_ledger(Request::new(GetLedgerRequest {}))
        .await
        .unwrap()
        .into_inner();

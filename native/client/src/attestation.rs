...
impl AttestationVerifier {
    ...
    async fn verify_gca_jwt(&self, token: &str) -> Result<GcaClaims, AttestationError> {
        ...
        let header =
            decode_header(token).map_err(|e| AttestationError::TokenVerification(e.to_string()))?;

        let kid = header.kid.ok_or_else(|| {
            AttestationError::TokenVerification("missing kid in JWT header".into())
        })?;
...
mod tests {
    ...
    async fn test_verify_untrusted_digest() {
        ...
        assert!(matches!(
            result,
            Err(AttestationError::UntrustedContainer(_))
        ));
    }
    ...
    async fn test_verify_missing_gca_token() {
        ...
        assert!(matches!(
            result,
            Err(AttestationError::MissingField("gca_token"))
        ));
    }
}
